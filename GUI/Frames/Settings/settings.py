...
class SettingsGroups(ctk.CTkTabview):
  def __init__(self,*args,**kwargs):
    ...
    self.mongodb.grid(column=0,row=0,padx=5,pady=5)
    ...
    self.payload.grid(column=0,row=0,padx=5,pady=5)
    self.ui = UI(self.tab('UI'))
    self.ui.grid(column=0,row=0,padx=5,pady=5)
    self.updates = Updates(self.tab('Updates'))
    self.updates.grid(column=0,row=0,padx=5,pady=5)
