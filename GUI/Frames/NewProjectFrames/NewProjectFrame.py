...
from GUI.Frames.NewProjectFrames.NewGithubFrame import NewGithubFrame
from GUI.Frames.NewProjectFrames.NewFrontendFrame import NewFrontendFrame
from GUI.Frames.NewProjectFrames.NewPluginFrame import NewPluginFrame
from GUI.Frames.NewProjectFrames.NewInfoFrame import NewInfoFrame
from GUI.Frames.NewProjectFrames.NewDBInfoFrame import NewDBInfoFrame
...
class NewProjectFrame(ctk.CTkFrame):
  def __init__(self,client:MongoDB,PPIM:Database,*args,**kwargs):
    ...
    self.githubInfo = NewGithubFrame(self.githubUser,self)
    self.githubInfo.grid(row=1,column=1,padx=(5,10),sticky='ew')

    self.frontendInfo = NewFrontendFrame(self.nodeVersions,self.payloadVersions,self.frontendTemplates,self.adminTemplates,self)
    self.frontendInfo.grid(row=2,column=0,padx=5,sticky='ew')

    self.backendInfo = NewDBInfoFrame(self.mongoDBVersions,self)
    self.backendInfo.grid(row=2,column=1,padx=(5,10),sticky='ew')

    self.projectInfo = NewInfoFrame(self.backendInfo, self.githubInfo, self.githubUser, self)
    self.projectInfo.grid(row=1,column=0,padx=5,sticky='ew')

    self.plugins = NewPluginFrame(self.plugins,self)
