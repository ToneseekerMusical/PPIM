...
class devTools(ctk.CTkTabview):
  def __init__(self, frontend, *args, **kwargs):
    ...
    self.startAdmin = ctk.CTkButton(self.tab('Dev Tools'),text='Start Admin',command=threading.Thread(target=lambda:self.AdminStart(self.frontend['adminPath'])).start)
